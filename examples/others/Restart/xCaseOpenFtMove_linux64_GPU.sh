...
${dualsphysicsgpu} -gpu ${dirout}/${name} ${dirout} -tmax:1
if [ $? -ne 0 ] ; then fail; fi
...
${floatinginfo} -dirdata ${diroutdata} -savedata ${dirout2}/FloatingMotion 
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/elevation
${measuretool} -dirdata ${diroutdata} -points ${name}_PointsElevation.txt -onlytype:-all,+fluid -elevation -savevtk ${dirout2}/PointsElevation -savecsv ${dirout2}/_Elevation
if [ $? -ne 0 ] ; then fail; fi
...
${dualsphysicsgpu} -gpu ${dirout}/${name} ${dirout} -partbegin:100 ${olddiroutdata}
if [ $? -ne 0 ] ; then fail; fi
...
${partvtkout} -dirdata ${diroutdata} -savevtk ${dirout2}/PartFluidOut -SaveResume ${dirout2}/ResumeFluidOut
if [ $? -ne 0 ] ; then fail; fi
