...
warning () { 
 echo MESHIN data file is missing. Run CasesSolWaveFt_Full before to generate the required data.
 fail 
}

# "name" and "dirout" are named according to the testcase

export name=CasesSolWaveFtOpen3D_MeshIN_x7_t3
...
if [ $option -eq 1 ]; then
# Check file with MESHIN data.
if [ ! -e CasesSolWaveFt_Full_out/GaugesMesh_Data-x7.mbi4 ]; then warning; fi
...
${gencase} ${name}_Def ${dirout}/${name} -save:all -save3vtk:0
...
${floatinginfo} -dirdata ${diroutdata} -savedata ${dirout}/_FloatingInfo
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/boundary
${boundaryvtk} -loadvtk AutoActual -motiondata ${diroutdata} -savevtkdata ${dirout2}/FtBox -onlytype:floating
if [ $? -ne 0 ] ; then fail; fi

${measuretool} -dirdata ${diroutdata} -pointsdef:ptels[x=8,z=0.3:0.005:0.8] -onlytype:-all,+fluid -elevation:0.4 -savecsv ${dirout}/_Elevation_x8
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/surface
${isosurface} -dirdata ${diroutdata} -saveiso ${dirout2}/Surface -vars:-all,vel,idp -iso_limits:7.1:-1:-1:13:1:3
if [ $? -ne 0 ] ; then fail; fi
