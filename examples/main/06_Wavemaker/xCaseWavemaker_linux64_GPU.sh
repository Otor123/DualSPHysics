...
${measuretool} -dirdata ${diroutdata} -points CaseWavemaker_PointsElevation.txt -onlytype:-all,+fluid -elevation -savevtk ${dirout2}/PointsElevation -savecsv ${dirout2}/_PointsElevation 
if [ $? -ne 0 ] ; then fail; fi

${measuretool} -dirdata ${diroutdata} -points CaseWavemaker_wg0_3D.txt -onlytype:-all,+fluid -elevation -savecsv ${dirout2}/_WG0 
if [ $? -ne 0 ] ; then fail; fi
