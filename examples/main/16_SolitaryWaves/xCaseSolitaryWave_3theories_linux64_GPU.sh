...
${measuretool} -dirdata ${diroutdata} -points wg_x8.txt -onlytype:-all,+fluid -elevation -savecsv ${dirout2}/eta_x8
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/measuretool
${measuretool} -dirdata ${diroutdata} -points wg_hx.txt -onlytype:-all,+fluid -elevation -savecsv ${dirout2}/eta_full -elevationoutput:pos -files:200-300:50
if [ $? -ne 0 ] ; then fail; fi
