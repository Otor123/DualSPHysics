...
%dualsphysicsgpu% -gpu %dirout%/%name% %dirout% -cellmode:2H
if not "%ERRORLEVEL%" == "0" goto fail

:postprocessing
set dirout2=%dirout%\particles
%partvtk% -dirdata %diroutdata% -savevtk %dirout2%/PartFluid -onlytype:-all,+fluid -vars:-all,+mk,+vel,+type,+rhop,+press
if not "%ERRORLEVEL%" == "0" goto fail
