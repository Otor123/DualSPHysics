...
${measuretool} -dirdata ${diroutdata} -points CaseWavemaker2D_wg1_2D.txt -onlytype:-all,+fluid -elevation -savecsv ${dirout2}/_wg1 -savevtk ${dirout2}/wg1
if [ $? -ne 0 ] ; then fail; fi

${measuretool} -dirdata ${diroutdata} -points CaseWavemaker2D_wg2_2D.txt -onlytype:-all,+fluid -elevation -savecsv ${dirout2}/_wg2 -savevtk ${dirout2}/wg2
if [ $? -ne 0 ] ; then fail; fi

${measuretool} -dirdata ${diroutdata} -points CaseWavemaker2D_wg3_2D.txt -onlytype:-all,+fluid -elevation -savecsv ${dirout2}/_wg3 -savevtk ${dirout2}/wg3
if [ $? -ne 0 ] ; then fail; fi
 
export dirout2=${dirout}/forces
${computeforces} -dirdata ${diroutdata} -onlyid:1616-1669  -viscoart:0.02 -savecsv ${dirout2}/WallForce -savevtk ${dirout2}/_WallForce
if [ $? -ne 0 ] ; then fail; fi
