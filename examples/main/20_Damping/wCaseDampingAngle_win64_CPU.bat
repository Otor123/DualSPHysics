...
REM %measuretool% -dirdata %diroutdata% -points elevation.txt -onlytype:-all,+fluid -elevation:0.4 -savecsv %dirout2%/_elevation -savevtk %dirout2%/elevation 
REM if not "%ERRORLEVEL%" == "0" goto fail

REM %measuretool% -dirdata %diroutdata% -points velocity.txt -onlytype:-all,+fluid -vars:-all,+vel -savecsv %dirout2%/_velocity -savevtk %dirout2%/velocity
REM if not "%ERRORLEVEL%" == "0" goto fail

REM set dirout2=%dirout%\surface
REM %isosurface% -dirdata %diroutdata% -saveslice %dirout2%/Slices 
REM if not "%ERRORLEVEL%" == "0" goto fail
