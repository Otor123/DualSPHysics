...
export name=CaseDampingCylinder
...
${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartFluid -onlytype:-all,+fluid -vars:-all,vel.m
if [ $? -ne 0 ] ; then fail; fi

${partvtk} -dirdata ${diroutdata} -saveenergy ${dirout}/_EnergyLeft -onlypos:-1:-1:0:0.7:1:1 -saveenergy ${dirout}/_EnergyRight -onlypos:0.7:-1:0:2:1:1
if [ $? -ne 0 ] ; then fail; fi
