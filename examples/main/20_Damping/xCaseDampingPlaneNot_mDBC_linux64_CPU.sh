...
export name=CaseDampingPlaneNot_mDBC
...
${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartFluid -onlytype:-all,+fluid -savevtk ${dirout2}/PartBound -onlytype:-all,+bound
if [ $? -ne 0 ] ; then fail; fi

${partvtk} -dirdata ${diroutdata} -saveenergy ${dirout}/_Energy
if [ $? -ne 0 ] ; then fail; fi

${partvtkout} -dirdata ${diroutdata} -savevtk ${dirout2}/PartFluidOut -SaveResume ${dirout2}/_ResumeFluidOut
if [ $? -ne 0 ] ; then fail; fi

${measuretool} -dirdata ${diroutdata} -pointsdef:ptels[x=3,y=0.2,z=0.3:0.005:0.5] -onlytype:-all,+fluid -elevation:0.4 -savecsv ${dirout}/_Elevation_x3
if [ $? -ne 0 ] ; then fail; fi

${measuretool} -dirdata ${diroutdata} -pointsdef:pt=3:0.2:0.3 -onlytype:-all,+fluid -vars:-all,+vel -savecsv ${dirout}/_Velocity_x3z03
if [ $? -ne 0 ] ; then fail; fi
