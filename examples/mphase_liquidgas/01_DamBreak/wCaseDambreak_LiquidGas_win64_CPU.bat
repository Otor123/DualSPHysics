...
set name=CaseDambreak_LiquidGas
...
mkdir %dirout2%
%measuretool% -dirin %dirout% -points %name%_PointsVelocity.txt -onlytype:-all,+fluid -vars:-all,+vel.x,+vel.m -savevtk %dirout2%/PointsVelocity -savecsv %dirout2%/_PointsVelocity
if not "%ERRORLEVEL%" == "0" goto fail

set dirout2=%dirout%\pressure
mkdir %dirout2%
%measuretool% -dirin %dirout% -points %name%_PointsPressure.txt -onlytype:-all,+fluid -vars:-all,+rhop,+press -savevtk %dirout2%/PointsPressure -savecsv %dirout2%/_PointsPressure
if not "%ERRORLEVEL%" == "0" goto fail
