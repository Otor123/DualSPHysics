...
export name=CaseFlowCylinderRe9500VRes
...
${partvtk} -dirdata ${dirout} -savevtk ${dirout2}/PartBound -onlytype:-all,+bound -vars:+idp,+vel,+rhop,+press 
if [ $? -ne 0 ] ; then fail; fi

${partvtk} -dirdata ${dirout} -savevtk ${dirout2}/PartFluid -onlytype:-all,+fluid -vars:+idp,+vel,+rhop,+press,+vor
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/forces
${computeforces} -dirdata ${dirout} -onlymk:10 -viscolam:1.0526e-05 -savecsv ${dirout2}/_ForceCylinder_VRes
if [ $? -ne 0 ] ; then fail; fi
