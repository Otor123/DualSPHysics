...
export name=CaseFloatingDuckVRes
...
${partvtk} -dirdata ${dirout} -savevtk ${dirout2}/PartFixed -onlytype:-all,+fixed -vars:-all,zone
if [ $? -ne 0 ] ; then fail; fi

${partvtk} -dirdata ${dirout} -savevtk ${dirout2}/PartFluid -onlytype:-all,+fluid -vars:press,zone
if [ $? -ne 0 ] ; then fail; fi

${partvtk} -dirdata ${dirout} -savevtk ${dirout2}/PartFloating -onlytype:-all,+floating -vars:press,zone
if [ $? -ne 0 ] ; then fail; fi
