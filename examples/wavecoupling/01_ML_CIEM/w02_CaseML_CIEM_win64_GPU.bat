...
%partvtk% -dirdata %diroutdata% -savevtk %dirout2%/PartMov -onlytype:-all,moving
if not "%ERRORLEVEL%" == "0" goto fail

rem WG measurements
set dirout2=%dirout%\measures
%measuretool% -dirdata %diroutdata% -points AWG5.txt -onlytype:-all,+fluid -elevation:0.4  -savecsv %dirout2%/AWG5
if not "%ERRORLEVEL%" == "0" goto fail
