...
set name=CaseInlet_AWAS_Coupling
set dirout=%name%_out
set diroutdata=%dirout%\data
set dirswash=Buckley_SWASH
...
if not exist %dirswash% echo ***Error: CSV files are missing, run the SWASH case first.
cd %dirswash%
copy CaseBuckley_awasIn.csv %dircopy%
copy CaseBuckley_inlet.csv %dircopy%
copy CaseBuckley_zsurf.csv %dircopy%
if not "%ERRORLEVEL%" == "0" goto fail
cd ..
