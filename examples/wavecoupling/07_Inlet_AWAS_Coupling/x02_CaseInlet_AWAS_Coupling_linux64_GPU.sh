...
export name=CaseInlet_AWAS_Coupling
export dirout=${name}_out
export diroutdata=${dirout}/data
export dirswash=Buckley_SWASH
...
if [ ! -e ${dirswash} ]; then echo ***Error: CSV files are missing, run the SWASH case first.; fi
cd ${dirswash}
cp CaseBuckley_awasIn.csv ${dircopy}
cp CaseBuckley_inlet.csv ${dircopy}
cp CaseBuckley_zsurf.csv ${dircopy}
if [ $? -ne 0 ] ; then fail; fi
cd ..
...
mkdir ${dirout2}
${measuretool} -dirdata ${diroutdata} -points WG.txt  -onlytype:-all,+fluid -elevation:0.4 -saveascii ${dirout2}/WG -savevtk ${dirout2}/WG
${measuretool} -dirdata ${diroutdata} -pointsVEL.txt -onlytype:-all,+fluid -vars:-all,+vel -saveascii ${dirout2}/EMS
# if [ $? -ne 0 ] ; then fail; fi
