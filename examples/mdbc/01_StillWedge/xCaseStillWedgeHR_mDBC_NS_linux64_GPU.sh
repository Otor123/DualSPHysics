...
${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartFluid -onlytype:-all,+fluid -vars:+idp,+vel,+rhop,+press,+energy
if [ $? -ne 0 ] ; then fail; fi
...
${partvtk} -dirdata ${diroutdata} -saveenergy ${dirout}/Energy
if [ $? -ne 0 ] ; then fail; fi
