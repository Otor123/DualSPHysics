...
%measuretool% -dirdata %diroutdata% -points points_cyl.txt -savecsv %dirout2%/VorPoint_DBC -vars:-all,+vor 
if not "%ERRORLEVEL%" == "0" goto fail

set dirout2=%dirout%\forces
%computeforces% -dirdata %diroutdata% -onlymk:10 -viscolam:0.001 -savecsv %dirout2%/_ForceCylinder_DBC
if not "%ERRORLEVEL%" == "0" goto fail
