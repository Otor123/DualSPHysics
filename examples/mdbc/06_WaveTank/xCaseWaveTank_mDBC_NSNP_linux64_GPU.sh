...
export dirout=${name}_mDBC_NSNP_out
...
${measuretool} -dirdata ${diroutdata} -points elevation.txt -onlytype:-all,+fluid -elevation:0.4 -savecsv ${dirout2}/_elevation -savevtk ${dirout2}/elevation 
if [ $? -ne 0 ] ; then fail; fi

${measuretool} -dirdata ${diroutdata} -points velocity.txt -onlytype:-all,+fluid -vars:-all,+vel -savecsv ${dirout2}/_velocity -savevtk ${dirout2}/velocity
if [ $? -ne 0 ] ; then fail; fi
