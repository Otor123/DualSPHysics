...
${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartBound -onlytype:-all,+bound -vars:+idp,+vel,+rhop,+press 
if [ $? -ne 0 ] ; then fail; fi
...
${measuretool} -dirdata ${diroutdata} -points PointsVelocity.txt -onlytype:-all,+fluid -vars:-all,+vel.x,+vel.m -savevtk ${dirout2}/PointsVelocity -savecsv ${dirout2}/_PointsVelocity
if [ $? -ne 0 ] ; then fail; fi
