...
export name=CaseSloshingSkillen
...
${dualsphysicsgpu} -gpu -mdbc_noslip:1 ${dirout}/${name} ${dirout}
if [ $? -ne 0 ] ; then fail; fi
...
${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartFluid -onlytype:-all,+fluid -vars:+idp,+vel,+rhop,+press
if [ $? -ne 0 ] ; then fail; fi
...
${boundaryvtk} -motiondata ${diroutdata} -saveposmotion:10:-0.448:0:0.093 ${dirout2}/PosCorrect 
if [ $? -ne 0 ] ; then fail; fi
# Executes MeasureTool to create a CSV file with pressure information at point close to the boundary.
${measuretool} -dirdata ${diroutdata} -pointspos ${dirout2}/PosCorrect_mk0010.csv -onlytype:-all,+fluid -vars:-all,+press,+kcorr -kcusedummy:0 -kclimit:0.5 -savevtk ${dirout2}/PointsPressure_Correct_002 -savecsv ${dirout2}/_PointsPressure_Correct_002
if [ $? -ne 0 ] ; then fail; fi
