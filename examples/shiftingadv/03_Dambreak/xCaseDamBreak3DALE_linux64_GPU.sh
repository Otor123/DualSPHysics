...
export name=CaseDamBreak3DAdvALE
...
${measuretool} -dirdata ${diroutdata} -points elevation.txt -onlytype:-all,+fluid -elevation:0.5 -savecsv ${dirout2}/_elevation  
if [ $? -ne 0 ] ; then fail; fi

${measuretool} -dirdata ${diroutdata} -points pressure.txt -onlytype:+all,+fluid -vars:-all,+press -kclimit:0.5 -savevtk ${dirout2}/pressure -savecsv ${dirout2}/_pressure 
if [ $? -ne 0 ] ; then fail; fi
