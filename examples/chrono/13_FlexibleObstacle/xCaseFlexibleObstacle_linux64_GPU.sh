...
export name=CaseFlexibleObstacle
...
${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartFixed -onlytype:-all,+fixed -vars:-all -files:0
if [ $? -ne 0 ] ; then fail; fi

${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartFluid -onlytype:-all,+fluid
if [ $? -ne 0 ] ; then fail; fi

${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartBeam -onlytype:-all,+floating
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/boundary
${boundaryvtk} -loadvtk AutoActual -motiondata ${diroutdata} -savevtkdata ${dirout2}/Gate.vtk -onlymk:11 
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/floatinginfo
${floatinginfo} -dirdata ${diroutdata} -onlymk:29 -savedata ${dirout2}/_FtInfo
if [ $? -ne 0 ] ; then fail; fi
