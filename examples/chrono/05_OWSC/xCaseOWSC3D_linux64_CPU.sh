...
${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartWavemaker -onlytype:-all,+moving
if [ $? -ne 0 ] ; then fail; fi

${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartFlap -onlytype:-all,+floating
if [ $? -ne 0 ] ; then fail; fi
...
${floatinginfo} -dirdata ${diroutdata} -onlymk:15 -savedata ${dirout2}/FlapMotion 
if [ $? -ne 0 ] ; then fail; fi
