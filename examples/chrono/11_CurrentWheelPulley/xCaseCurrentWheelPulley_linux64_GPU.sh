...
${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartWheel -onlytype:-all,+floating -vars:+mk
if [ $? -ne 0 ] ; then fail; fi
...
${floatinginfo} -dirdata ${diroutdata} -onlymk:60 -savedata ${dirout2}/Wheel 
if [ $? -ne 0 ] ; then fail; fi

${floatinginfo} -dirdata ${diroutdata} -onlymk:61 -savedata ${dirout2}/Gear 
if [ $? -ne 0 ] ; then fail; fi
