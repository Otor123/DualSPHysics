...
${boundaryvtk} -loadvtk AutoActual -motiondatatime ${diroutdata} -savevtkdata ${dirout2}/MotionGate -onlymk:10 -savevtkdata ${dirout2}/Box.vtk -onlytype:fixed
if [ $? -ne 0 ] ; then fail; fi

export dirout2=${dirout}/particles
${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartFixed -onlytype:-all,+fixed -files:0
if [ $? -ne 0 ] ; then fail; fi

${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartFluid -onlytype:-all,+fluid
if [ $? -ne 0 ] ; then fail; fi

${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartGate -onlymk:10
if [ $? -ne 0 ] ; then fail; fi

${partvtk} -dirdata ${diroutdata} -savevtk ${dirout2}/PartStructure -onlymk:12
if [ $? -ne 0 ] ; then fail; fi
