...
inline bool IsBound(const TpParticlesNew type){ return(type<TpPartFluid); }

/// Returns if particle type is fluid or not.
inline bool IsFluid(const TpParticlesNew type){ return(type==TpPartFluid); }


/// Returns the string code of particle type.
inline const char* TpPartGetStrCode(const TpParticlesNew type){
  switch(type){
...
inline TpParticlesNew TpPartGetType(std::string strcode){
