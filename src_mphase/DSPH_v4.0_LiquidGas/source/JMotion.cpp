...
//==============================================================================
void JMotion::SetDirIn(const std::string &dirin){
  Dirin=fun::GetDirWithSlash(dirin);
}
...
void JMotion::ObjAdd(unsigned id,unsigned idparent,int ref){
  ...
  if(ObjGetPointer(id))RunException(met,"Cannot add an object with an existing id.");
  if(ref>=0&&ObjGetPointerByRef(ref))RunException(met,"Cannot add an existing object with a real reference(higher than or equal to zero).");
  JMotionObj* parent=(idparent? ObjGetPointer(idparent): NULL);
  if(idparent&&!parent)RunException(met,"Parent object missing.");
  JMotionObj* obj=new JMotionObj(id,parent,ref,&Dirin);
...
void JMotion::EventAdd(unsigned objid,unsigned movid,double timestart,double timefinish){
  ...
  if(!mov)RunException(met,"Missing movement inside the object.");
...
void JMotion::MovAddRectilinearFile(unsigned objid,unsigned id,unsigned nextid,double time,const std::string &file,int fields,int fieldtime,int fieldx,int fieldy,int fieldz){
  ...
  MovAdd(objid,new JMotionMovRectFile(id,nextid,time,file,fields,fieldtime,fieldx,fieldy,fieldz));
}
//==============================================================================
...
{
  MovAdd(objid,new JMotionMovRotFile(id,nextid,time,angdegrees,AxisAdd(objid,axisp1,axisp2),file));
}
...
void JMotion::CheckLinkMovs()const{
  for(unsigned c=0;c<Objs.size();c++)if(Objs[c]->LinkMovs())RunException("CheckLinkMovs","A movement refers to another non-existent movement within the object."); 
}
...
//==============================================================================
void JMotion::ReadXml(JXml *jxml,TiXmlNode* node,unsigned &id,unsigned idp){
...
// Carga configuracion de motion en formato xml
//==============================================================================
void JMotion::ReadXml(JXml *jxml,const std::string &path,bool checkexists){
  Reset();
  unsigned id=0;
  TiXmlNode* node=jxml->GetNode(path,false);
  if(node){
    ReadXml(jxml,node,id,0);
    CheckLinkMovs();
  }
  else if(checkexists){
...
//==============================================================================
void JMotion::LoadFileXml(const std::string &file,const string &path){
  JXml jxml;
  jxml.LoadFile(file);
  ReadXml(&jxml,path);
}
