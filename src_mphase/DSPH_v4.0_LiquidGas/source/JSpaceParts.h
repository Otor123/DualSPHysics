...
public:
  const TpParticlesNew Type;    ///<Type of particle.
  const bool Bound;          ///<Indicates whether a particle is boundary or not.

  JSpacePartBlock(const JSpaceProperties* properties,TpParticlesNew type,const char* name
...
public:
  ...
  ~JSpaceParts();
  void Reset();
  unsigned Count(TpParticlesNew type)const;
  unsigned Count()const;
  unsigned CountBlocks()const{ return(unsigned(Blocks.size())); }
  unsigned CountBlocks(TpParticlesNew type)const;
  ...
  std::string GetMkList(TpParticlesNew type)const;
