...
// Constructor.
//==============================================================================
JMotionMovActive::JMotionMovActive(double start,double eventfinish,JMotionMov* mov,const std::string* dirin):EventFinish(eventfinish),Dirin(dirin){
...
void JMotionMovActive::DfReset(){
  delete[] DfTimes; DfTimes=NULL;
  delete[] DfPos;   DfPos=NULL;
  delete[] DfAng;   DfAng=NULL;
  DfPosType=false;
  DfSize=0; DfCount=0; DfIndex=0;
...
//==============================================================================
void JMotionMovActive::DfResize(unsigned size){
  //printf("JMotionMovActive::DfResize> %u --> %u\n",DfSize,size);
  if(size>DfSize){
    double* times=new double[size];
    tdouble3* pos=NULL;
    double* ang=NULL;
    if(DfPosType)pos=new tdouble3[size];
    else         ang=new double[size];
    if(DfSize){
      memcpy(times,DfTimes,sizeof(double)*DfSize);
      if(DfPosType)memcpy(pos,DfPos,sizeof(tdouble3)*DfSize);
      else         memcpy(ang,DfAng,sizeof(double)*DfSize);
      delete[] DfTimes;
      delete[] DfPos;
      delete[] DfAng;
    }
...
//==============================================================================
void JMotionMovActive::DfLoadFilePos(std::string file,const int fields,const int fieldtime,const int fieldx,const int fieldy,const int fieldz){
  const char met[]="DfLoadFilePos";
  if(int(file.find("/"))<0 && int(file.find("\\"))<0)file=(*Dirin)+file; //-If only name of file then add Dirin.
  //-Carga datos del fichero.
  JReadDatafile rdat;
  rdat.LoadFile(file,DFSIZEMAX);
  ...
  for(unsigned r=0;r<rows;r++){
    ...
    DfTimes[r]=time; DfPos[r]=TDouble3(posx,posy,posz);
    //printf("[%u]>  t:%f  x:%f\n",r,time,posx);
  }
  DfCount=rows;
  if(DfCount<2)RunException(met,"Cannot be less than two positions.",file);
}

//==============================================================================
// Carga y configura datos del movimiento a partir de fichero de datos.
//==============================================================================
void JMotionMovActive::DfLoadFileAng(std::string file,bool angdegrees){
  const char met[]="DfLoadFileAng";
  if(int(file.find("/"))<0 && int(file.find("\\"))<0)file=(*Dirin)+file; //-If only name of file then add Dirin.
  //-Carga datos del fichero.
  JReadDatafile rdat;
  rdat.LoadFile(file,DFSIZEMAX);
  ...
  DfResize(rows);
  for(unsigned r=0;r<rows;r++){
    DfTimes[r]=rdat.ReadNextDouble();
    DfAng[r]=rdat.ReadNextDouble();
    //printf("FileData[%u]>  t:%f  ang:%f\n",r,time,ang);
  }
  if(!angdegrees)for(unsigned r=0;r<rows;r++)DfAng[r]=DfAng[r]*TODEG; //-Convierte de radianes a grados.
  DfCount=rows;
  if(DfCount<2)RunException(met,"Cannot be less than two angles.",file);
}
...
void JMotionMovActive::DfConfig(bool postype){
  ...
  if(DfPosType){
    ...
    DfLoadFilePos(mv->File,mv->Fields,mv->FieldTime,mv->FieldX,mv->FieldY,mv->FieldZ);
    DfLastPos=DfPos[0];
  }
  else{
    JMotionMovRotFile* mv=(JMotionMovRotFile*)Mov;
    DfLoadFileAng(mv->File,mv->AngDegrees);
    DfLastAng=DfAng[0];
  }
}
...
// Constructor.
//==============================================================================
JMotionObj::JMotionObj(unsigned id,JMotionObj* parent,int ref,const std::string* dirin):Id(id),Parent(parent),Ref(ref),Dirin(dirin){
...
//==============================================================================
bool JMotionObj::LinkMovs(){
  bool err=false;
  for(unsigned c=0;c<Movs.size()&&!err;c++){
    JMotionMov* mov=Movs[c];
    if(mov->NextId){
      mov->SetNextMov(MovGetPointer(mov->NextId));
      if(!mov->NextMov)err=true;
    }
  }
  for(unsigned c=0;c<Children.size()&&!err;c++)err=Children[c]->LinkMovs();
  return(err);
}
...
void JMotionObj::BeginEvent(double start,double eventfinish,JMotionMov* mov){
  JMotionMovActive* amov=new JMotionMovActive(start,eventfinish,mov,Dirin);
...
//printf("ReajusteDeInicio dtmov: %G     dtmov2: %G\n",-(amov->Start-timestep2),dtmov);
          ...
          if(dtmov>0||amov->Flash)switch(mov->Type){
            case JMotionMov::Rectilinear:{
              JMotionMovRect *mv=(JMotionMovRect*)mov;
...
//            printf("***JMotionMov::Rectilinear\n");
            ...
            case JMotionMov::RectilinearAce:{
              JMotionMovRectAce *mv=(JMotionMovRectAce*)mov;
              double t=(amov->Flash? -mv->Time: dtmov);
            ...
            case JMotionMov::Rotation:{
              JMotionMovRot *mv=(JMotionMovRot*)mov;
              double t=(amov->Flash? -mv->Time: dtmov);
              ModPos.Rotate(mv->VelAng*t,mv->Axis->P1,mv->Axis->P2);
              modif=true;
            }break;
            case JMotionMov::RotationAce:{
              JMotionMovRotAce *mv=(JMotionMovRotAce*)mov;
              double t=(amov->Flash? -mv->Time: dtmov);
            ...
            case JMotionMov::Circular:{
              JMotionMovCir *mv=(JMotionMovCir*)mov;
              double t=(amov->Flash? -mv->Time: dtmov);
            ...
            case JMotionMov::CircularAce:{
              JMotionMovCirAce *mv=(JMotionMovCirAce*)mov;
              double t=(amov->Flash? -mv->Time: dtmov);
              double at=mv->AceAng*t;
            ...
            case JMotionMov::RectilinearSinusoidal:{
              JMotionMovRectSinu *mv=(JMotionMovRectSinu*)mov;
              double t=(amov->Flash? -mv->Time: dtmov);
            ...
            case JMotionMov::RotationSinusoidal:{
              JMotionMovRotSinu *mv=(JMotionMovRotSinu*)mov;
              double t=(amov->Flash? -mv->Time: dtmov);
            ...
            case JMotionMov::CircularSinusoidal:{
              JMotionMovCirSinu *mv=(JMotionMovCirSinu*)mov;
              double t=(amov->Flash? -mv->Time: dtmov);
              double ph=amov->PhaseUni;
              double ang=mv->Ampl*sin(ph); 
              ph+=double(mv->Freq*(PI+PI)*t); 
  ...
  if(modif){
    ...
    Moving=false;
    printf("Motion> Obj[%d] goes to inactive...\n",Id);
  }
